...
public interface UserPostRepository extends CrudRepository<UserPost, UserPostId> {
    ...
    final String upsertQuery = "INSERT INTO user_post (user_id, post_id, is_liked) "
            + "VALUES (:userId, :postId, :isLiked) "
            + "ON DUPLICATE KEY UPDATE is_liked = VALUES(is_liked)";

    final String getAllPostsWithDetailsQuery = "SELECT p.id AS id, p.created_date as createdDate, p.image, p.text, "
            ...
            + "FROM posts p "
            + "LEFT JOIN user_post up ON p.id = up.post_id "
